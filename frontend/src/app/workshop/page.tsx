...
const WORKSHOP_INFO = [
  { icon: '🎓', title: 'Workshop AjiStat', desc: 'Hands-on analisis data dengan SPSS, SmartPLS, NVivo, R, Python. Pakai dataset riset nyata.', prog: 'AjiStat' },
  { icon: '💼', title: 'Workshop AjiBiz', desc: 'Praktik langsung business model canvas, analisis pasar, dan strategi pemasaran bisnis.', prog: 'AjiBiz' },
  { icon: '🎤', title: 'Workshop AjiPR', desc: 'Latihan public speaking, presentasi profesional, dan media relation langsung dengan feedback.', prog: 'AjiPR' },
];
  ...
  return (
    <>
      ...
      <div className="bg-[#0C1A45] relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#2568B5]/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-[#4FA8D8]/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4" />
          ...
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
              {STATS.map(({ icon: Icon, value, label }) => (
                <div key={label} className="bg-white/8 border border-white/12 rounded-2xl p-4">
                  <Icon className="w-5 h-5 text-[#4FA8D8] mx-auto mb-2" />
            ...
            <div className="relative max-w-md mx-auto">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/40 text-sm focus:outline-none focus:border-[#4FA8D8] transition-colors"
              />
            </div>
          </div>
        </div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <p className="text-[#2568B5] text-sm font-semibold uppercase tracking-widest mb-2">Jenis Workshop</p>
            ...
            {WORKSHOP_INFO.map((w) => (
              <div key={w.title} className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-lg transition-all hover:-translate-y-1">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs bg-[#162660]/10 text-[#162660] px-2 py-0.5 rounded-full font-semibold">{w.prog}</span>
                </div>
                <p className="text-gray-500 text-sm mb-4">{w.desc}</p>
                <a href={WA_LINK(`Halo, saya ingin info tentang ${w.title}`)}
                  target="_blank" rel="noopener noreferrer"
                  className="text-sm text-[#2568B5] font-semibold hover:underline">
                  💬 Tanya Info →
                </a>
              </div>
            ))}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-gray-50 rounded-2xl">
              ...
              <a href={WA_LINK('Halo, saya ingin info jadwal workshop Aji Institute')}
                target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-[#162660] text-white font-bold px-6 py-3 rounded-xl hover:bg-[#2568B5] transition-colors">
