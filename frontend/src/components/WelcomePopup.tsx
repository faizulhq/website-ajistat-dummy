...
/* ─────────────────────────────────────────────
   Shape of data returned by  GET /api/popup/active/
   ───────────────────────────────────────────── */
interface PopupData {
  id: number;
  image: string;          // full URL
  badge?: string;         // e.g. "Info Terbaru" — optional
  badge_color?: string;   // hex, e.g. "#F0A500"
  title: string;
  subtitle?: string;
...
export function WelcomePopup({ site }: { site: 'main' | 'ajistat' }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/cms/popup_active/`)
      ...
      .catch(() => {/* silently fail — no popup shown */});
  }, [site]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Gambar flyer — full ratio, framed */}
          <div
          ...
          {/* Label bawah */}
          <p className="text-white/25 text-[10px] tracking-widest uppercase mt-1">aji-institute.com</p>
        </div>
